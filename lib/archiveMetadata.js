...
// Extensions to NEVER consider as EPUB (avoid "repub" matching)
...
function analyzeArchiveFiles(metadata) {
  ...
  for (const f of files) {
    ...
    // EPUB candidates - use strict validation (never matches "repub")
    if (isValidEpubFile(f)) {
      epubCandidates.push({ name, format: 'epub', size });
    }
    // PDF candidates (prefer "Text PDF" over scanned) - use strict validation
...
async function batchCheckReadability(items, maxProbes = 20) {
  ...
  for (const item of items) {
    if (item.archive_id || (item.direct_url && item.direct_url.includes('archive.org'))) {
      ...
      // Non-IA items (gutenberg, oapen, openstax) keep their existing readable status
      // Handle both boolean and string 'true'/'false' values
      const existingReadable = item.readable === true || item.readable === 'true';
      const existingUnreadable = item.readable === false || item.readable === 'false';
      otherItems.push({
        ...item,
        readable: existingUnreadable ? 'false' : (existingReadable ? 'true' : 'maybe'),
      });
    }
  }
