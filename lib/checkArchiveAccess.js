 ...
 * Instead of filtering out restricted items, marks them with head_check_failed flag
 * so they can still be shown with "Open Source Link" option
 * 
 ...
 * @returns {Promise<Array>} Array with head_check_failed flag added to restricted items
 */
async function filterRestrictedArchiveItems(items, maxChecks = 15) {
  ...
  // Mark items as head_check_failed instead of filtering them out
  const checkedItems = checkResults.map(r => ({
    ...r.item,
    head_check_failed: !r.accessible,
    // Update reason if check failed
    reason: !r.accessible ? 'borrow_required' : r.item.reason,
    external_only: !r.accessible ? true : r.item.external_only,
  }));
  
  const restrictedCount = checkResults.filter(r => !r.accessible).length;
  const accessibleCount = checkResults.filter(r => r.accessible).length;
  
  console.log(`[search] ia access check: accessible=${accessibleCount} restricted=${restrictedCount} unchecked=${unchecked.length}`);
  
  // Return all items (other + checked + unchecked)
  return [...otherItems, ...checkedItems, ...unchecked];
}
