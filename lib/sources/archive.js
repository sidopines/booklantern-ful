...
async function search(q, page = 1) {
  try {
    ...
    const params = {
      q: query,
      fl: 'identifier,title,creator,year,language,format,access-restricted-item',
      rows,
      page: page,
      output: 'json',
    };
    ...
    for (const doc of docs) {
      ...
      const restrictedFlag = doc['access-restricted-item'];
      const isRestricted = restrictedFlag === true || restrictedFlag === 'true' || restrictedFlag === 1;
      const access = isRestricted ? 'restricted' : 'public';
