...
// GET /admin/video-genres — list + form (render EJS)
...
// POST /admin/video-genres — create
router.post('/', async (req, res) => {
  if (!supabase) return res.redirect(303, '/admin/video-genres');
  const name = String(req.body.name || '').trim();
  if (!name) return res.redirect(303, '/admin/video-genres');
  try {
  ...
  return res.redirect(303, '/admin/video-genres');
});

// POST /admin/video-genres/:id/rename
router.post('/:id/rename', async (req, res) => {
  if (!supabase) return res.redirect(303, '/admin/video-genres');
  const id = String(req.params.id || '').trim();
  const name = String(req.body.name || '').trim();
  if (!id || !name) return res.redirect(303, '/admin/video-genres');
  try {
    const { error } = await supabase
      .from('video_genres')
      .update({ name })
...
// POST /admin/video-genres/:id/delete
router.post('/:id/delete', async (req, res) => {
  if (!supabase) return res.redirect(303, '/admin/video-genres');
  const id = String(req.params.id || '').trim();
  if (!id) return res.redirect(303, '/admin/video-genres');
  try {
